...
/*
Debug run in psql:
MODULE_PATHNAME /usr/lib/postgresql/10/lib/median.so

CREATE OR REPLACE FUNCTION _median_transfn(state internal, val anyelement)
RETURNS internal
AS '/usr/lib/postgresql/10/lib/median.so', 'median_transfn'
LANGUAGE C IMMUTABLE;

CREATE OR REPLACE FUNCTION _median_finalfn(state internal, val anyelement)
RETURNS anyelement
AS '/usr/lib/postgresql/10/lib/median.so', 'median_finalfn'
LANGUAGE C IMMUTABLE;
